...
do
  ...
  -- bmp.help
  local help = function()
    print("BMP180 query module")
    print("  dofile(\"bmp180.lua\").help()")
    print("  bmp = dofile(\"bmp180.lua\")")
    print("  isok = bmp.init(sda, scl, oss)")
    print("  {temp, pa, hgmm, alt} = bmp.read()")
  end

  -- bmp.init
  local init = function(sda, scl, oss)
  ...
  local read = function()
    ...
    t = (X2 + X1 + 8) / 16
    -- get P
    w8(0xF4, 0x34 + 64 * samples)
    tmr.delay((4 + 3 ^ samples) * 1000)
    ...
    local X3 = X1 + X2
    local B3 = ((AC1 * 4 + X3) * 2 ^ samples + 2) / 4
    X1 = AC3 * B6 / 8192
    X2 = (B1 * (B6 * B6 / 4096)) / 65536
    X3 = (X1 + X2 + 2) / 4
    local B4 = AC4 * (X3 + 32768) / 32768
    local B7 = (p - B3) * (50000 / 2 ^ samples)
    p = B7 / B4 * 2
    X1 = (p / 256) ^ 2
    X1 = (X1 * 3038) / 65536
    X2 = (-7357 * p) / 65536
    p = p + (X1 + X2 + 3791) / 16
    -- Celsius, Hg mm, Pa, Meters
    local result = {}
    result.temp = t / 10.0
    result.pa = p
    result.hgmm = p / 133.322
    result.alt = 44330 * (1 - (p / 101325) ^ (1 / 5.255))
    return result
  end
